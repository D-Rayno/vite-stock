...
//!
//! Uses `printpdf` 0.5.x — all shapes are drawn with the `Line` primitive.
//! printpdf's `Mm` wraps `f64`, so **all geometry values are f64** throughout
//! this module.  Do not use f32 literals for sizes or positions.
...
// ── Page geometry (all f64 — Mm wraps f64) ────────────────────────────────────
...
pub struct PdfCanvas {
    ...
    /// Current vertical position in mm (f64 — matches Mm type).
    pub cursor_y: f64,
}
...
impl PdfCanvas {
    ...
    /// `y` = top of the band, `h` = band height (both in mm, f64).
    ...
    pub fn kv_row(&mut self, key: &str, value: &str, size: f64) {
        ...
        // Approximate right-align of value (char_width ≈ size_pt × 0.55 × pt→mm)
    ...
    pub fn header(
        ...
        if !nif.is_empty() {
            layer.use_text(
                ...
                8.0, Mm(MARGIN_L), Mm(self.cursor_y), &fn_,
            );
        }
        layer.use_text(
            &format!("Ref: {doc_ref}  |  {date}"),
            8.0, Mm(right_x), Mm(self.cursor_y), &fn_,
        );
        self.cursor_y -= 5.0;
    ...
    pub fn table(
        ...
        // Convert col widths to f64 for geometry
        let col_widths: Vec<f64> = cols.iter().map(|(_, w, _)| *w).collect();
        ...
        let mut cx = MARGIN_L + 1.0_f64;
        for ((label, _, align), &w) in cols.iter().zip(col_widths.iter()) {
            layer.set_fill_color(white());
            // FIX: dereference &&str → &str for Into<String> to be satisfied
            let x = if *align == 1 {
...
pub fn build_dain_statement(data: &DainStatementData<'_>) -> Result<Vec<u8>, String> {
    ...
    if data.entries.is_empty() {
        ...
        layer.use_text(
            ...
            9.0, Mm(MARGIN_L), Mm(c.cursor_y), &fn_,
        );
        c.cursor_y -= LH_BODY;
    }
